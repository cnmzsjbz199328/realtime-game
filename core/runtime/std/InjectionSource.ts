...
// Auto-generated by scripts/generate-injection-source.mjs — DO NOT EDIT MANUALLY
// Run `npm run generate-injection` to regenerate after modifying Vector.ts or RetroAudio.ts
...
class RetroAudio {
  ...
  note(freq, duration = 0.5, type = "sine", vol = 0.5) {
    ...
    const osc = this.ctx.createOscillator();
    const g = this.ctx.createGain();
    const safeDuration = Math.min(Math.max(duration, 0.05), 5);
    osc.connect(g);
    g.connect(this.master);
    osc.type = type;
    const NOTE_FREQS = {
    ...
    g.gain.linearRampToValueAtTime(1e-3, t + safeDuration);
    osc.start(t);
