...
**Vector is now single-source**: `core/runtime/std/Vector.ts` is the only copy. `InjectionSource.ts` is auto-generated by `scripts/generate-injection-source.mjs` (runs as `prebuild`). `HeadlessBrowserValidator` imports `Vector` directly. To change Vector behaviour, edit only `Vector.ts` — the next build propagates it everywhere.
