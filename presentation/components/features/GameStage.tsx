...
export const GameStage: React.FC<GameStageProps> = ({ status, game, onCrash, onSave }) => {
    ...
    return (
        <section className={`flex-1 min-h-[500px] transition-all duration-700 ${status === AgentStatus.DEPLOYED ? 'opacity-100 translate-y-0' : 'opacity-40 translate-y-4 grayscale'}`}>
            {status === AgentStatus.DEPLOYED && game ? (
                <div className="h-full border-2 border-purple-500/30 rounded-2xl p-1 shadow-[0_0_50px_rgba(168,85,247,0.15)] bg-zinc-900/50 backdrop-blur-sm relative group">
                    <GameHarness gameDef={game} onCrash={onCrash} />

                    <button
                        onClick={handleSave}
                        disabled={saved}
                        className="absolute top-4 right-4 bg-zinc-900/80 backdrop-blur text-white p-3 rounded-full hover:bg-zinc-800 transition-all border border-zinc-700 z-50 group-hover:opacity-100 opacity-0"
                        title="Save to Collection"
                    >
                        {saved ? <span className="text-green-500">❤️</span> : <span className="text-zinc-400 hover:text-red-500">🤍</span>}
                    </button>
                </div>
            ) : (
